...
- Food and exercise log entries — delivered as a soft void (FTY-321): the entry and its derived items, clarification questions, evidence sources, and corrections are excluded from every read but retained in storage until the user/account-deletion cascades remove them (see the as-built status below).
- Body weight entries.
...
sources, corrections) from every read model — including derived summaries — but
