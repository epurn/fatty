...
semantic, wording, field, table, vocabulary, or ordering change. This page keeps
the `## Exact Evidence Upgrade — FTY-306` heading as a forwarding pointer, so
sibling-doc and code references naming that section still resolve; the evidence
core (source hierarchy, fallback rule, evidence record, fact schema, provider
capability/status, search/fetch boundaries) and the still-inline sub-contracts
are unchanged.
...
`kind = barcode` half of the [**Exact Evidence Upgrade**](exact-evidence-upgrade.md)). It reuses the existing
...
[**Exact Evidence Upgrade — FTY-306**](exact-evidence-upgrade.md) (extracted to
its own page by FTY-396, version 13 above); backend implementation is
FTY-307–FTY-309, mobile consumption FTY-310–FTY-313.
...
**Moved to [exact-evidence-upgrade.md](exact-evidence-upgrade.md).** The
`Make it exact` proposal/apply sub-contract — eligibility, the proposal read
shape (`proposal_ref` / `kind` / `quality` / `failure_reason` / preview /
costability), exact-vs-fallback quality, the source-replacement apply semantics,
and its authorization/privacy rules — now lives on its own page. This heading is
retained so references naming this section still resolve.
